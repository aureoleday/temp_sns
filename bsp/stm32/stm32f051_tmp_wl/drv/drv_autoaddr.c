...
#define DOWN_I    GET_PIN(A, 6)
#define DOWN_O    GET_PIN(A, 7)
#define UP_I      GET_PIN(A, 4)
#define UP_O      GET_PIN(A, 5)
...
{
    uint8_t i_sts,o_sts;
    i_sts = rt_pin_read(UP_I);
    o_sts = rt_pin_read(UP_O);
    rt_kprintf("sr\trxd\ttxd\tcnt\tfsm\tauto_addr\tUP_I\tUP_O\t\n");
    rt_kprintf("0x%x\t0x%x\t0x%x\t0x%x\t0x%x\t0x%x\t\t0x%x\t0x%x\t\n",autoaddr_inst.sr,autoaddr_inst.rxd,autoaddr_inst.txd,autoaddr_inst.cnt,autoaddr_inst.fsm,autoaddr_inst.auto_addr,i_sts,o_sts);
}
...
void autoaddr_thread_entry(void* parameter)
{
    ...
    autoaddr_inst.fsm = FSM_INIT;
        ...
        conf_inst.bm_field.addr_fsm = autoaddr_inst.fsm;
				rt_thread_mdelay(100);
		}
}
